import { HZEngineCore, Storage, UI } from "hzengine-core";
import React from "react";
import ReactDOM from 'react-dom/client'

export function registerReactView<PropType extends Storage.Saveable<PropType>>(
  core: HZEngineCore,
  name: string,
  component: React.ComponentType<any>
) {
  const ReactView = class extends UI.View<PropType, any> {
    div: HTMLDivElement | null = null;
    root: ReactDOM.Root | null = null;
    parent: HTMLDivElement | null = this.core.ui.getLayer(this.layer)
    ?.widgetFactory;
  protected onCreate(prop: any): void {
    console.log("create view. parent", this.parent);

    this.div = document.createElement("div");
    // div.style.zIndex = z_index.toString();
    this.div.style.position = "absolute";
    this.div.style.width = "100%";
    this.div.style.height = "100%";
    this.div.style.left = "0px";
    this.div.style.top = "0px";

    this.parent?.append(this.div);
    this.root = ReactDOM.createRoot(this.div)
    this.root.render(React.createElement(component, prop, null))
    this.core.debug.log(`ReactView [${this.name}] created`, prop);
  }
  protected onCommit(prop: any): void {
    this.core.debug.log(`ReactView [${this.name}] committed`, prop);
    this.root!.render(React.createElement(component, prop, null))
  }
  protected onDestroy(): void {
    this.core.debug.log(`ReactView [${this.name}] destroyed`);
    this.root!.unmount();
    this.div!.remove();
  }
};
  core.ui.registerView(name, ReactView);
}
