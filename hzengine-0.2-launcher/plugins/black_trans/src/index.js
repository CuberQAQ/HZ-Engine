...
export default function BlackTrans(core) {
        ...
        onCreate(prop) {
            ...
            if (this.enableAnim) {
                this._animationId = animationPlugin.createTempAnimation({
                    ...
                    onFrame: (props) => {
                        this._widget.setAlpha(props.alpha);
                    },
                    onEnd: () => {
                        if (this.onCreated) {
                            this.onCreated();
            ...
            if (this.enableAnim) {
                this._animationId = animationPlugin.createTempAnimation({
                    profile: [
                        { frame: { alpha: 1 } },
                        {
                            time: this.prop.out_time,
                            wrapper: this.prop.out_wrapper,
                            frame: { alpha: 0 },
                        },
                    ],
                    onFrame: (props) => {
    ...
    let activeView = null;
    return {
        show({ cb, layer = "overlay", in_time = 0.3, out_time = 0.3, in_wrapper = "easein", out_wrapper = "easeout", }) {
            if (activeView) {
            ...
            activeView.onCreated = cb;
        },
        hide({ cb }) {
            if (activeView) {
                activeView.onDestroyed = cb;
                core.ui.destroyView(activeView);
                activeView = null;
            }
        },
